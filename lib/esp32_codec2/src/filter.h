...
struct quisk_cfFilter {     // Structure to hold the static data for FIR filters
  ...
  float _Complex *cpxCoefs;  // complex filter coefficients
  ...
  float _Complex *cSamples;  // storage for old samples
  float _Complex *ptcSamp;   // next available position in cSamples
  float _Complex *cBuf;      // auxiliary buffer for interpolation
};

extern int quisk_cfInterpDecim(float _Complex *, int, struct quisk_cfFilter *,
                               int, int);
...
extern void quisk_ccfFilter(float _Complex *, float _Complex *, int,
