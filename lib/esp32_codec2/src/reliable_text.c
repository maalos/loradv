...
typedef struct {
  ...
  float _Complex inbound_pending_syms[(RELIABLE_TEXT_UW_LENGTH_BITS +
...
static int reliable_text_ldpc_decode(reliable_text_impl_t* obj, char* dest) {
  ...
  float _Complex deinterleavedSyms[LDPC_TOTAL_SIZE_BITS / 2];
...
static void reliable_text_freedv_callback_rx_sym(void* state,
  ...
  obj->inbound_pending_syms[obj->sym_index] = (float _Complex)sym;
    ...
    if (uw_bits_valid) {
      ...
      if (reliable_text_ldpc_decode(obj, rawStr) != 0) {
        ...
        memset(&obj->inbound_pending_syms, 0,
               sizeof(float _Complex) * LDPC_TOTAL_SIZE_BITS / 2);
