...
void quisk_filt_cfInit(struct quisk_cfFilter *filter, float *coefs, int taps) {
  ...
  filter->cSamples = (float _Complex *)MALLOC(taps * sizeof(float _Complex));
  memset(filter->cSamples, 0, taps * sizeof(float _Complex));
...
int quisk_cfInterpDecim(float _Complex *cSamples, int count,
  ...
  if (count > filter->nBuf) {  // increase size of sample buffer
    ...
    filter->cBuf =
        (float _Complex *)MALLOC(filter->nBuf * sizeof(float _Complex));
  }

  memcpy(filter->cBuf, cSamples, count * sizeof(float _Complex));
  nOut = 0;
...
\*---------------------------------------------------------------------------*/
#if 0
int quisk_ccfInterpDecim(float _Complex * cSamples, int count, struct quisk_cfFilter * filter, int interp, int decim) {
    // Interpolate by interp, and then decimate by decim.
    // This uses the complex coefficients of filter (not the real).  Samples are complex.
    int i, k, nOut;
    float _Complex * ptCoef;
    float _Complex * ptSample;
    float _Complex csample;
    ...
    if (count > filter->nBuf) {    // increase size of sample buffer
        ...
        if (filter->cBuf)
            FREE(filter->cBuf);
        filter->cBuf = (float _Complex *)MALLOC(filter->nBuf * sizeof(float _Complex));
    }
...
void quisk_cfTune(struct quisk_cfFilter *filter, float freq) {
  ...
  if (!filter->cpxCoefs)
    filter->cpxCoefs =
        (float _Complex *)MALLOC(filter->nTaps * sizeof(float _Complex));
...
void quisk_ccfFilter(float _Complex *inSamples, float _Complex *outSamples,
