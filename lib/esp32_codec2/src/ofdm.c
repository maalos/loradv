...
static float cnormf(float _Complex);
...
static void dft(struct OFDM *, float _Complex *, float _Complex *);
static void idft(struct OFDM *, float _Complex *, float _Complex *);
static float _Complex vector_sum(float _Complex *, int);
static int est_timing(struct OFDM *, float _Complex *, int, int, float *, int *,
                      int);
static float est_freq_offset_pilot_corr(struct OFDM *, float _Complex *, int,
                                        int);
 ...
 */
static const float _Complex qpsk[] = {1.0f + 0.0f * I, 0.0f + 1.0f * I,
                                     0.0f - 1.0f * I, -1.0f + 0.0f * I};

static const float _Complex qam16[] = {
...
static float cnormf(float _Complex val) {
 ...
 */
float _Complex qpsk_mod(int *bits) { return qpsk[(bits[1] << 1) | bits[0]]; }
 ...
 * 11 | 10
 */
void qpsk_demod(float _Complex symbol, int *bits) {
  float _Complex rotate = symbol * cmplx(ROT45);

  bits[0] = crealf(rotate) <= 0.0f;
  bits[1] = cimagf(rotate) <= 0.0f;
}

float _Complex qam16_mod(int *bits) {
  return qam16[(bits[3] << 3) | (bits[2] << 2) | (bits[1] << 1) | bits[0]];
}

void qam16_demod(float _Complex symbol, int *bits) {
  float dist[16];
  int i;
...
struct OFDM *ofdm_create(const struct OFDM_CONFIG *config) {
  ...
  ofdm->pilot_samples =
      (float _Complex *)MALLOC(sizeof(float _Complex) * ofdm->samplespersymbol);
  assert(ofdm->pilot_samples != NULL);

  ofdm->rxbuf = (float _Complex *)MALLOC(sizeof(float _Complex) * ofdm->nrxbuf);
  assert(ofdm->rxbuf != NULL);
  for (int i = 0; i < ofdm->nrxbuf; i++) ofdm->rxbuf[i] = 0;

  ofdm->pilots =
      (float _Complex *)MALLOC(sizeof(float _Complex) * (ofdm->nc + 2));
   ...
   */
  ofdm->rx_sym = MALLOC(sizeof(float _Complex) * (ofdm->ns + 3));
  ...
  ofdm->rx_np = MALLOC(sizeof(float _Complex) * (ofdm->rowsperframe * ofdm->nc));
  ...
  ofdm->tx_uw_syms =
      MALLOC(sizeof(float _Complex) * (ofdm->nuwbits / ofdm->bps));
  ...
  float _Complex *temp = MALLOC(sizeof(float _Complex) * ofdm->m);
  assert(temp != NULL);
 ...
 */
static void idft(struct OFDM *ofdm, float _Complex *result,
                 float _Complex *vector) {
  int row, col;
  ...
  for (row = 1; row < ofdm->m; row++) {
    float _Complex c = cmplx(ofdm->tx_nlower * ofdm->doc * row);
    float _Complex delta = cmplx(ofdm->doc * row);
 ...
 * This algorithm was optimized for speed
 */
static void dft(struct OFDM *ofdm, float _Complex *result,
                float _Complex *vector) {
  int row, col;
  ...
  for (col = 0; col < (ofdm->nc + 2); col++) {
    ...
    float _Complex c = cmplxconj(tval);
    float _Complex delta = c;
...
static float _Complex vector_sum(float _Complex *a, int num_elements) {
...
static float _Complex ofdm_complex_dot_product(float _Complex *left,
                                              float _Complex *right,
                                              int numSamples) {
  float _Complex result = 0;
  ...
  for (int i = 0; i < numBlocks; i++) {
    ...
    /* Shift pointers forward by 4 (2 float _Complexs). */
    leftPtr += 4;
    rightPtr += 4;
  }
 ...
 * +/- 25 Hz for 700D).
 */
static int est_timing(struct OFDM *ofdm, float _Complex *rx, int length,
                      int fcoarse, float *timing_mx, int *timing_valid,
                      int step) {
  float _Complex corr_st, corr_en;
  ...
  float _Complex wvec_pilot[ofdm->samplespersymbol];
 ...
 */
static float est_freq_offset_pilot_corr(struct OFDM *ofdm, float _Complex *rx,
  ...
  // look for peak
  for (int f = st; f < en; f++) {
    float _Complex corr_st = 0.0f;
    float _Complex corr_en = 0.0f;
    float tmp = TAU * f / ofdm->fs;
    float _Complex delta = cmplxconj(tmp);
    float _Complex w = cmplxconj(0.0f);
    int i;

    for (i = 0; i < ofdm->samplespersymbol; i++) {
      // "mix" down (correlate) the pilot sequences from frame with 0 Hz offset
      // pilot samples
      float _Complex csam = wvec_pilot[i] * w;
 ...
 * ----------------------------------------------
 */
void ofdm_txframe(struct OFDM *ofdm, float _Complex *tx,
                  float _Complex *tx_sym_lin) {
  float _Complex aframe[ofdm->np * ofdm->ns][ofdm->nc + 2];
  float _Complex asymbol[ofdm->m];
  float _Complex asymbol_cp[ofdm->samplespersymbol];
  int i, j, k, m;
 ...
 * PAPR */
void ofdm_hilbert_clipper(struct OFDM *ofdm, float _Complex *tx, size_t n) {
  ...
  if (ofdm->tx_bpf_en) {
    assert(ofdm->tx_bpf != NULL);
    float _Complex tx_filt[n];

    quisk_ccfFilter(tx, tx_filt, n, ofdm->tx_bpf);
    memmove(tx, tx_filt, n * sizeof(float _Complex));
  }
...
void ofdm_mod(struct OFDM *ofdm, COMP *result, const int *tx_bits) {
  ...
  float _Complex *tx =
      (float _Complex *)result;  // complex has same memory layout
  float _Complex tx_sym_lin[length];
  int dibit[2];
  int s, i;
...
int ofdm_sync_search(struct OFDM *ofdm, COMP *rxbuf_in) {
  ...
  memmove(&ofdm->rxbuf[0], &ofdm->rxbuf[ofdm->nin],
          (ofdm->nrxbuf - ofdm->nin) * sizeof(float _Complex));
  memmove(&ofdm->rxbuf[(ofdm->nrxbuf - ofdm->nin)], rxbuf_in,
          ofdm->nin * sizeof(float _Complex));
...
static float est_timing_and_freq(struct OFDM *ofdm, int *t_est, float *foff_est,
                                 float _Complex *rx, int Nrx,
                                 float _Complex *known_samples, int Npsam,
  ...
  *foff_est = 0.0;
  for (float afcoarse = fmin; afcoarse <= fmax; afcoarse += fstep) {
    ...
    float _Complex mvec[Npsam];
    for (int i = 0; i < Npsam; i++) {
      float _Complex ph = cmplx(w * i);
      mvec[i] = conjf(known_samples[i] * ph);
    }
    for (int t = 0; t < Ncorr; t += tstep) {
      float _Complex corr = ofdm_complex_dot_product(&rx[t], mvec, Npsam);
...
static void burst_acquisition_detector(struct OFDM *ofdm, float _Complex *rx,
                                       int n, float _Complex *known_sequence,
...
static int ofdm_sync_search_burst(struct OFDM *ofdm) {
  ...
  burst_acquisition_detector(ofdm, ofdm->rxbuf, st,
                             (float _Complex *)ofdm->tx_preamble, &pre_ct_est,
  ...
  if (ofdm->postambledetectoren)
    burst_acquisition_detector(ofdm, ofdm->rxbuf, st,
                               (float _Complex *)ofdm->tx_postamble,
...
static int ofdm_sync_search_core(struct OFDM *ofdm) {
  if (ofdm->rx_bpf_en) {
    assert(ofdm->rx_bpf != NULL);
    float _Complex *rxbuf_in = &ofdm->rxbuf[(ofdm->nrxbuf - ofdm->nin)];
...
void ofdm_demod(struct OFDM *ofdm, int *rx_bits, COMP *rxbuf_in) {
  float _Complex *rx =
      (float _Complex *)&rxbuf_in[0];  // complex has same memory layout
  int i, j;
...
static void ofdm_demod_core(struct OFDM *ofdm, int *rx_bits) {
  ...
  float _Complex aphase_est_pilot_rect;
 ...
 */
float ofdm_esno_est_calc(float _Complex *rx_sym, int nsym) {
  float sig_var = 0;
...
void ofdm_get_demod_stats(struct OFDM *ofdm, struct MODEM_STATS *stats,
                          float _Complex *rx_syms, int Nsymsperpacket) {
  stats->Nc = ofdm->nc;
  ...
  for (int c = 0; c < ofdm->nc; c++) {
    for (int r = 0; r < Nrowsperpacket; r++) {
      float _Complex rot = rx_syms[r * ofdm->nc + c] * cmplx(ROT45);
...
void ofdm_assemble_qpsk_modem_packet_symbols(struct OFDM *ofdm,
                                             float _Complex modem_packet[],
                                             COMP payload_syms[],
                                             uint8_t txt_bits[]) {
  float _Complex *payload =
      (float _Complex *)&payload_syms[0];  // complex has same memory layout
...
void ofdm_disassemble_qpsk_modem_packet(struct OFDM *ofdm,
                                        float _Complex rx_syms[],
                                        float rx_amps[], COMP codeword_syms[],
                                        float codeword_amps[],
                                        short txt_bits[]) {
  float _Complex *codeword =
      (float _Complex *)&codeword_syms[0];  // complex has same memory layout
  int Nsymsperpacket = ofdm->bitsperpacket / ofdm->bps;
  int Nuwsyms = ofdm->nuwbits / ofdm->bps;
  int Ntxtsyms = ofdm->ntxtbits / ofdm->bps;
  int dibit[2];
...
void ofdm_disassemble_qpsk_modem_packet_with_text_amps(
    struct OFDM *ofdm, float _Complex rx_syms[], float rx_amps[],
 ...
 */
void ofdm_extract_uw(struct OFDM *ofdm, float _Complex rx_syms[],
...
// hilbert clipper
void ofdm_clip(float _Complex tx[], float clip_thresh, int n) {
  float _Complex sam;
  float mag;
  int i;
